...
type MockEventSubscriber[A AckableMessage] struct {
	mock.Mock
}

// Subscribe provides a mock function with given fields: topicName, handler, options
func (_m *MockEventSubscriber[A]) Subscribe(topicName string, handler MessageHandler[EventMessage], options ...SubscriberOption) error {
	...
	var r0 error
	if rf, ok := ret.Get(0).(func(string, MessageHandler[EventMessage], ...SubscriberOption) error); ok {
...
func (_m *MockEventSubscriber[A]) Unsubscribe() error {
	ret := _m.Called()
...
func NewMockEventSubscriber[A AckableMessage](t interface {
	mock.TestingT
	Cleanup(func())
}) *MockEventSubscriber[A] {
	mock := &MockEventSubscriber[A]{}
	mock.Mock.Test(t)
