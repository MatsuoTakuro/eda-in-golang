...
type MockCommandSubscriber[A AckableMessage] struct {
	mock.Mock
}

// Subscribe provides a mock function with given fields: topicName, handler, options
func (_m *MockCommandSubscriber[A]) Subscribe(topicName string, handler MessageHandler[CommandMessage], options ...SubscriberOption) error {
	...
	var r0 error
	if rf, ok := ret.Get(0).(func(string, MessageHandler[CommandMessage], ...SubscriberOption) error); ok {
...
func (_m *MockCommandSubscriber[A]) Unsubscribe() error {
	ret := _m.Called()
...
func NewMockCommandSubscriber[A AckableMessage](t interface {
	mock.TestingT
	Cleanup(func())
}) *MockCommandSubscriber[A] {
	mock := &MockCommandSubscriber[A]{}
	mock.Mock.Test(t)
