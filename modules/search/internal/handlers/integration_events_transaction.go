...
func RegisterIntegrationEventHandlersTx(container di.Container) (err error) {
	evtMsgHandler := am.RawMessageHandlerFunc(func(ctx context.Context, msg am.IncomingMessage) (err error) {
	...
	if err = subscriber.Subscribe(customerspb.CustomerAggregateChannel,
		evtMsgHandler,
		am.MessageFilter{
	...
	if err = subscriber.Subscribe(orderingpb.OrderAggregateChannel,
		evtMsgHandler,
		am.MessageFilter{
	...
	if err = subscriber.Subscribe(storespb.ProductAggregateChannel,
		evtMsgHandler,
		am.MessageFilter{
	...
	if err = subscriber.Subscribe(storespb.StoreAggregateChannel,
		evtMsgHandler,
		am.MessageFilter{
