...
import (
	...
	"eda-in-golang/modules/notifications/migrations"
)
...
func run() (err error) {
	...
	if err = s.MigrateDB(migrations.NotificationsFS); err != nil {
		return err
	}
	s.Mux().Mount("/", http.FileServer(http.FS(web.WebUI)))
	// call the module composition root
	if err = notifications.Root(s.Runner().Context(), s); err != nil {
		return err
	}
