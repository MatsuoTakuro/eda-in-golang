...
import (
	...
	jstm "github.com/nats-io/nats.go/jetstream"
...
func (s *integrationEventsTestSuite) SetupSuite() {
	...
	s.reg = registry.New()
	if err = storespb.RegisterMessages(s.reg); err != nil {
		s.T().Fatal(err)
	}
	...
	_, err = s.js.CreateStream(ctx, jstm.StreamConfig{
		Name:     streamName,
...
func (s *integrationEventsTestSuite) SetupTest() {
	...
	s.stream = am.NewEventStream(s.reg, jetstream.NewStream("baskets", streamName, s.js, logger))
